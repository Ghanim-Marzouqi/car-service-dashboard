...
import { getRegions, getWillayats, getAllServices, createGarage, getGarageById, updateGarage } from "../services";
import { getLoggedInUserState } from "../store";

const EditOwnerGarage = () => {
  ...
  useEffect(() => {
    setForm((prevState) => ({ ...prevState, owner_id: user.id }))
  ...
  }, [id, history, form.region_id, user.id]);
    ...
    <>
      ...
      <Form className="align-self-center mt-5">
        ...
        <FormGroup>
          ...
          <Input type="text" name="owner" readOnly value={user.name} onChange={() => { }}></Input>
        </FormGroup>
        <FormGroup>
